...
const DeleteVideoButton5 = ({ videoId }) => {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await fetch(`http://localhost:4000/leet/auth/videos5/${id}`, requestOptions);
  ...
  useEffect(() => {
    ...
    const fetchVideos = async () => {
      ...
      const fetchedVideos = await fetch('http://localhost:4000/leet/auth/videos5', {
        headers: {
...
export default DeleteVideoButton5;
